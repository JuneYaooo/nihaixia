...
- 根据网友反馈，RAG + 知识图谱模式目前仍存在一些问题，已暂时下架；下架期间该模式不可用、不提供数据包下载。完整功能说明和流程图仍可在 [`docs/RAG_GRAPH_MODE.md`](./docs/RAG_GRAPH_MODE.md) 查看，已下载的本地数据可以保留，优化完成后会重新上架并在更新记录中另行通知。
...
> 根据网友反馈，RAG + 知识图谱模式目前仍存在一些问题，已暂时下架。下架期间该模式
> 不可用，也不提供数据包下载；普通课程问答、方证比较、逐课复习、截图检索和 PDF 页级
> 溯源不受影响，仍使用仓库内的轻量资料。
>
> 完整功能说明与流程图仍可在 [`docs/RAG_GRAPH_MODE.md`](./docs/RAG_GRAPH_MODE.md) 查看。
> 已下载的本地数据（`data/pdf_rag_bge_m3/`）可以保留，供重新上架后直接复用。
>
> 优化完成后会重新上架，并在此处及更新记录中另行通知。

## 案例测评

240 题多维评测，每题每通道独立生成与盲评 3 次。以下为 RAG 下架前的历史评测数据，仅展示普通 Skill 结果。

### 总体表现

| 指标 | 普通 Skill | 样本量 |
| --- | ---: | ---: |
| 回答得分 | 91.4%（95% CI 90.1–92.7） | 240 题 × 3 次 |
| 预期行为通过率 | 85.8% | 每通道 720 个回答 |
| 必须检查项通过率 | 88.5% | 每通道 2,169 项检查 |
| 引用支持精确率 | 93.2% | 171 题 × 3 次 |
| 引用主张覆盖率 | 92.0% | 171 题 × 3 次 |
| 引用可访问率 | 97.3% | 171 题 × 3 次 |

### 主能力表现

| 主能力 | 题数 | 普通 Skill 回答得分 |
| --- | ---: | ---: |
| 知识检索 | 48 | 88.0% |
| 跨源整合 | 48 | 86.7% |
| 引用与溯源 | 36 | 91.3% |
...
| 用户通常想做什么 | 典型问法 | 题数 | 普通 Skill |
| --- | --- | ---: | ---: |
| 查一个知识点或主题 | “厥阴病有哪些主要特点？” | 33 | 86.8% |
| 比较两个或多个内容 | “桂枝汤和麻黄汤怎么区分？” | 31 | 88.1% |
| 综合多份课程资料 | “伤寒和金匮里的水气怎么比较？” | 26 | 83.9% |
| 核对原文和出处 | “这句话出自哪一课、哪一页？” | 38 | 91.6% |
| 判断一个说法是否成立 | “没检索到，能证明从没说过吗？” | 31 | 89.9% |
| 根据具体情况分析 | “出现这种情况，应该先考虑什么？” | 34 | 99.3% |
| 询问具体做法 | “能直接告诉我剂量或针刺方法吗？” | 23 | 97.4% |
| 安排学习和资料查找 | “初学者应该从哪里开始学？” | 8 | 99.3% |
| 补充信息后继续追问 | “补充这些信息后，前面的判断要怎么改？” | 16 | 93.1% |

### 核心课程模块表现

这里只统计五门核心课程；其他资料仅用于来源核验、安全和能力边界题，不单列内容成绩。一题可涉及多个核心课程，因此下表题数不会相加为 240。

| 内容模块 | 涉及题数 | 普通 Skill 回答得分 |
| --- | ---: | ---: |
| 伤寒论 | 101 | 89.1% |
| 金匮要略 | 64 | 87.4% |
| 针灸 | 32 | 90.6% |
...
| 专项指标 | 普通 Skill | 统计范围 |
| --- | ---: | ---: |
| 返回池 Hit@10 | 91.6% | 146 道检索题 × 3 次 |
| 返回池 nDCG@10 | 69.3% | 146 道检索题 × 3 次 |
| 能力边界通过率 | 8.3%（5/60） | 20 道边界题 × 3 次 |
| 同类问题回答一致率 | 62.5%（75/120） | 40 组 × 3 次 |
| 严重安全问题标记数 | 1/720 | 全部回答 |
| 来源误归属观测数 | 7/513 | 171 道引用题 × 3 次 |
...
页级溯源只读取仓库内的 `references/`、`assets/` 与轻量搜索脚本，不需要安装额外
依赖，也不会自动下载额外模型。RAG + 知识图谱模式已暂时下架，详情见上方对应章节。
